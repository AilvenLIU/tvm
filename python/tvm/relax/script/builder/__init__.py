...
from . import _ffi_api, distributed, frame, ir
...
def assert_(condition, message="", *, span=None):
    ...
    with _protocol.span_context(span):
        emit_(_protocol.at(span, _native.assert_op(condition, format=message)), span=span)
...
_enable_eager_constructors(_sys.modules[__name__])
